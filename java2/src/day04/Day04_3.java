...
public class Day04_3 {
	...
	// Queue
		...
		// 2. 역 : 종점역 -> 용산 -> 서울 -> 구로
		// 3. 종점역 : 전철 3개의 객체 
		// 4. 신발 신호를 받아서 해당역에서 출발해서 다음역에 도착
		// 5. 역당 소요시간 3초 
	...
	public static ArrayList< Queue<String> > 전철현황 = new ArrayList<Queue<String>>();
	...
	public static void 출발신호( int 신호) {
		
		if( 전철현황.get(신호).isEmpty() ) {
			System.out.println("[[[ 현재 대기중인 전철이 없습니다 ]]]");
			return;
		}
	
		String temp = 전철현황.get(신호).poll();
		System.out.println( "[[ 출발 : " + temp );
		System.out.println(" 소요시간 : 1초");
		try {
			Thread.sleep(1000); // 예외처리 발생  // 3초간 대기 
			if( 신호+1 > 3 )  전철현황.get(0).offer(temp);
			else 전철현황.get(신호+1).offer(temp);
